# -*- coding: utf-8 -*-
"""
Created on Thu Jul 06 11:10:11 2017
...
#    c = 2.5
#    Weight_Decay = c / float(batchdata.shape[0])
    model.add(Flatten())
    model.add(Dense(512))
    model.add(Activation('relu'))
...
def AL_EN(model,proba,data,label,batch_data,batch_label,num,flag):
    ...
    if num==batch_size:
        ...
        print("num",num,entropy)
        threshold = 0.005-0.00008*(batch_data.shape[0]-2000)/batch_size
        pse_index =[]
        if threshold>0:
            for i in range(entropy.shape[0]):
                if entropy[i,]>0 and entropy[i,]<threshold:
                    pse_index.append(i)
            n=len(pse_index)
            n1 = int(0.1*n)
    ...
    if num==batch_size:
        ...
        if threshold>0:
            for i in range(BvSB.shape[0]):
                if BvSB[i,]>0 and BvSB[i,]<threshold:
                    pse_index.append(i)
            n=len(pse_index)
            n1 = int(0.1*n)
            print("pse...........: ",pse_index[0:n1])
...
def initial(X_train_All,y_train_All):
    ...
    X_Pool = X_train_All[20000:50000, :, :, :]
    Y_Pool = y_train_All[20000:50000]
...
#
#        print(flag.count(0))
#    path = 'cifar10_random_bs256_0728_%d.json'%ct
...
#       log5['val_loss'].append(loss)
#       x += 1
#    path='cifar10_Entropy_bs256_CEAL_0728_%d.json'%ct
#    with open(path, 'w') as f:
#       json.dump(log5,f)
#    ct += 1
#    del model_active1
...
#       log6['val_loss'].append(loss)
#       x += 1
#    path='cifar10_BvSB_bs256_CEAL_0728_%d.json'%ct
#    with open(path, 'w') as f:
#       json.dump(log6,f)
#    ct += 1
#    del model_active2
...
#       log7['val_loss'].append(loss)
#       x += 1
#    path='cifar10_LC_bs256_CEAL_0728_%d.json'%ct
#    with open(path, 'w') as f:
#       json.dump(log7,f)
#    ct += 1
#    del model_active3
